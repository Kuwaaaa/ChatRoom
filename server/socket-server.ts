...
interface RoomUser {
  ...
  isHost: boolean;
  // P2P 扩展字段（预留）
  peerConnections?: string[];  // 已连接的 peer ID
  networkStats?: {
    bandwidth: number;
    latency: number;
    natType: string;
  };
  role?: 'super' | 'normal';  // 超级节点 or 普通节点
}
    ...
    socket.on('join-room', ({ roomId, userId, userName, networkStats }) => {
      ...
      if (!rooms.has(roomId)) {
        ...
        rooms.set(roomId, {
          users: new Map(),
          videoState: {
            url: room?.video_url || null,
            time: room?.video_time || 0,
...
// 评估用户角色（P2P 扩展）
function evaluateUserRole(networkStats?: any): 'super' | 'normal' {
  if (!networkStats) return 'normal';
